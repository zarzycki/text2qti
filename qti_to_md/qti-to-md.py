...
def parse_qti_to_markdown(qti_file):
    try:
        ...
        for section in root.findall('.//qti:section', namespace):
            ...
            for index, item in enumerate(items):
                ...
                correct_answer = item.find('.//qti:varequal', namespace)
                correct_ident = correct_answer.text if correct_answer is not None else None

                # Handle the answer choices with labels a), b), c), etc.
                ...
                for answer_index, response in enumerate(item.findall('.//qti:response_label', namespace)):
                    ...
                    # Determine the label based on the index and mark the correct answer
                    label = answer_labels[answer_index]
                    if correct_ident == ident:
